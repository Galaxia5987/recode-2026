...
* **Execution Blocking:** Most commands inside mechanisms should be written to block (using a `while` loop with a `!this` yield) until they meet their setpoint. This ensures sequential composition works correctly by default. Callers can use `.fork()` if they wish to run the command asynchronously.
...
* **Yielding:** Any continuous action or waiting loop inside a command *must* call `!this` (which translates to `coroutine.yield()`) at the end of each iteration to prevent locking the main thread.
...
* **Composition:** Ensure proper use of `+` for sequential execution and `+[]` for parallel execution within a `noRequirements` or mechanism block.

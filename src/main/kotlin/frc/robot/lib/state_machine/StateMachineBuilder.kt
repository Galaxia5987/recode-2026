...
class StateMachineBuilder<E : Enum<E>>(val name: String, val log: Boolean = true) {
    ...
    private fun getState(enumVal: E): StateMachine.State {
        return stateMap[enumVal] ?: error("State $enumVal was used in a transition but never defined.")
    }
    ...
    inner class TransitionCondition(val source: E, val condition: Trigger, val transitionType: TransitionType)
    inner class MultiTransitionCondition(val sources: Array<out E>, val condition: Trigger)
    inner class CompleteTransitionWrapper(val source: E)

    infix fun E.on(trigger: Trigger): TransitionCondition = TransitionCondition(this, trigger, TransitionType.ON)
    infix fun E.on(condition: () -> Boolean): TransitionCondition = on(Trigger(condition))

    infix fun E.completeAnd(trigger: Trigger): TransitionCondition = TransitionCondition(this, trigger, TransitionType.COMPLETE_AND)
    infix fun E.completeAnd(condition: () -> Boolean): TransitionCondition = completeAnd(Trigger(condition))
    ...
    inline fun <reified E : Enum<E>> allOf(): Array<out E> = enumValues<E>() as Array<out E>

    infix fun Array<out E>.on(trigger: Trigger): MultiTransitionCondition = MultiTransitionCondition(this, trigger)
    infix fun Array<out E>.on(condition: () -> Boolean): MultiTransitionCondition = on(Trigger(condition))

    infix fun MultiTransitionCondition.switchTo(target: E) {
        val sourceStates = this.sources.map { getState(it) }.toTypedArray()
        stateMachine.switchFromAny(*sourceStates).to(getState(target)).`when`(this.condition)
    }

...
enum class IntakeState{
    ...
    companion object {
        val stateMachine = buildStateMachine<IntakeState>("Intake State Machine"){
            IDLE{
            ...
            IDLE on outtakeButton.and(!inExtendedAllianceZone).and(isInDoubleFeedingZone) switchTo OUTTAKING
            PUMPING on !isShooting switchTo IDLE
            PUMPING on intakeButton switchTo INTAKING
            PUMPING on outtakeButton.and(isInDoubleFeedingZone).and(!inExtendedAllianceZone) switchTo OUTTAKING
            INTAKING on !intakeButton switchTo IDLE
            INTAKING on outtakeButton.and(isInDoubleFeedingZone).and(!inExtendedAllianceZone) switchTo OUTTAKING
            OUTTAKING on outtakeButton.negate().or(isInDoubleFeedingZone.negate().and(!inExtendedAllianceZone)) switchTo IDLE
        }
    }
}